...
<template>
  <div class="grid grid-cols-2 gap-y-6 place-items-center my-10 md:grid-cols-4">
    ...
    >
      <div
        class="w-24 h-24 md:w-16 md:h-16 bg-[#f18a00] rounded-full flex justify-center items-center lg:w-24 lg:h-24 cursor-pointer"
        @click="$router.push(`/disciplinas/${discipline.attributes.link}`)"
      >
        <img
          class="w-12 md:w-10"
      ...
      <div class="text-[var(--color-tertiary)] gap-2 text-center">
        <h4 class="font-black text-3xl md:text-xl lg:text-3xl cursor-pointer" @click="$router.push(`/disciplinas/${discipline.attributes.link}`)">
