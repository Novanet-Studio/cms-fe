<template>
  <div class="swiper">
    <swiper :autoplay="{ delay: 6000, disableOnInteraction: false }" :slider-per-view="1" :space-between="0" loop
      ...
      <swiper-slide v-for="(articulo, index) in articulos" :key="index">
        <highlight estilo="highlight-slide" :title="articulo.attributes.titulo"
          :description="excerpt(articulo.attributes.descripcion, 480)"
          :image="articulo.attributes.imagen.data.attributes.url"
          :alt="articulo.attributes.imagen.data.attributes.alternativeText" :url="`/blog/${articulo.attributes.slug}`" />
      </swiper-slide>
    </swiper>
  </div>
  ...
  <section class="blog">
    <div class="blog__wrapper">
      ...
      <template v-else>
        <article class="blog-item" v-for="(post, index) in articulos" :key="index">
          <div class="blog-item__img-container">
            <nuxt-picture :src="post.attributes.imagen.data.attributes.url"
              :alt="post.attributes.imagen.data.attributes.alternativeText" class="blog-item__imagen" />
          </div>
          <div class="blog-item__info">
            ...
            <app-button class="button--yellow button--small blog-item__boton" :url="`/blog/${post.attributes.slug}`">
              Leer más
            </app-button>
          </div>
        </article>
