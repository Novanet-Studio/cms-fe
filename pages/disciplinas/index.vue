<template>
  <div class="swiper">
    <swiper :slider-per-view="1" :space-between="0" loop navigation :modules="[Autoplay, Navigation, Pagination]">
      <swiper-slide v-for="(slide, index) in slides" :key="index">
        <nuxt-img :src="slide.attributes.url" sizes="xs:100vw sm:640 md:768 lg:1024 xl:1280 xxl:1920" />
      </swiper-slide>
    </swiper>
  </div>
    ...
    <div class="disciplinas__container">
      ...
      <div class="cards__wrapper">
        <div v-for="(disciplina, index) in disciplinas" :key="index">
          <nuxt-link :to="{ path: `/disciplinas/${disciplina.attributes.link}`, replace: true }">
            <card :logo="disciplina.attributes.icono.data.attributes.url" :alternativeText="
              disciplina.attributes.icono.data.attributes.alternativeText
            " :title="disciplina.attributes.nombre" description="Ver información y planes" />
          </nuxt-link>
        </div>
      </div>
    </div>
